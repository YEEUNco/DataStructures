...
bool findPathBack(tree root, tree x, vector<int>& path) {
	...
	//cout << "your code here \n";
	if(root==x || findPathBack(root->left,x,path)==true || findPathBack(root->right,x,path)==true){
...
bool findPath(tree node, tree x, vector<int>& path) {
	...
	if(findPath(node->left,x, path)==true || findPath(node->right, x, path)==true)
		return true;
...
bool isBST(tree x, int min, int max) {
	...
	//cout << "your code here\n";
	vector<int> temp, forSort;
	inorder(x,temp);
	inorder(x, forSort);
	sort(forSort.begin(), forSort.end());
	int size = temp.size();
	for(int i=0; i<size; i++){
		if(temp[i]!=forSort[i]) return false;
	}
	if(temp[0]>=min && temp[size]<=max) return true;
...
void BTtoBST(tree root) {
	...
	vector<int> tempKey;
	inorder(root,tempTree);
	inorder(root, tempKey);
	sort(tempKey.begin(), tempKey.end());
	//int num = temp.size();
	//assert(num == size(root));
	for(int i=0; i<tempKey.size(); i++){
		tempTree[i]->key = tempKey[i];
	}
