...
void rehash(Hash*& ht, int new_size) {  
    ...
    //cout << "your code here\n";
    list<wordcount>* new_list = new list<wordcount>[new_size];
    ...
    Hash* new_ht =new Hash {new_size};
    int num = ht->tablesize;
    ht->tablesize = new_size;
    for(int i=0; i<num; i++){
        for(it = ht->hashtable[i].begin(); it != ht->hashtable[i].end(); it++){
            int index = hashfunction(ht,it->first);
            new_list[index].insert(new_list[index].begin(),make_pair(it->first,it->second) );
        }
...
bool erase(Hash*& ht, string key) {
    ...
    for(it = ht->hashtable[index].begin(); it != ht->hashtable[index].end(); ){
        if(it->first == key){
