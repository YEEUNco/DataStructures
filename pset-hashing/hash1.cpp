...
void rehash(Hash*& ht) {
    ...
    new_ht->nelements = ht->nelements;
    new_ht->threshold = ht->threshold;
    delete ht;
    ht = new_ht;
...
bool erase(Hash*& ht, string key) {
   ...
   //cout << "your code here\n";
    int keyH = hashfunction(ht,key);
    list<string>::iterator it;
                        
    // for loop using it, then return true if erased successfully.
    //cout << "your code here\n";
    for(it = ht->hashtable[keyH].begin(); it != ht->hashtable[keyH].end(); ){
        if(*it == key){
            ht->hashtable[keyH].erase(it);
            return true;
        }
        else it++;
    }
