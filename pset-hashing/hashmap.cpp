...
bool insert(unordered_map<string, int>& ht, string key) {
	...
	//cout << "your code here \n";
	unordered_map<string,int>::iterator got = ht.find(key);
	if(got == ht.end())
...
void show_range(unordered_map<string, int>& ht, vector<int>& bucket_idx, size_t from, size_t to) {
	...
	for(int i=from; i<to; i++){
		cout << "\t[" << bucket_idx[i] << "]  ";
		for(auto it = ht.begin(bucket_idx[i]); it!=ht.end(bucket_idx[i]); it++){
	...
	do {
		...
		case 'f':  // find
			key = GetString("\tEnter a key to find: ");
			{
				...
				if(ht.end() != ht.find(key)){
					cout << key << ": " << ht.find(key)->second << " \t";
				}
				else{
